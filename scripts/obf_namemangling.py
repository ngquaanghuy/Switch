...
class NameCollector(ast.NodeVisitor):
    ...
    def __init__(self):
        ...
        self.local_attrs = set()  # attributes/methods defined in local classes
    ...
    def visit_ClassDef(self, node):
        self.names.add(node.name)
        # Collect methods and attributes defined in this class
        for item in node.body:
            if isinstance(item, (ast.FunctionDef, ast.AsyncFunctionDef)):
                self.local_attrs.add(item.name)
            elif isinstance(item, ast.Assign):
                for target in item.targets:
                    if isinstance(target, ast.Name):
                        self.local_attrs.add(target.id)
        self.generic_visit(node)
...
def obfuscate(source):
    ...
    skip.update(collector.local_attrs)  # Don't rename class methods/attrs (unsafe without full analysis)
